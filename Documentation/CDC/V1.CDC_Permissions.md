...
Groupe (max 15 membres + adhérents illimités)
  └-> Organisation simple (30 membres) — déblocage Admin/GM après ticket
        └-> Organisation avancée (50 membres) — déblocage Admin/GM après ticket
              └-> Nation (75 gratuit + achat jusqu'à 100 membres) — déblocage Admin/GM après ticket
                  (voir CDC_NationModule.md — système désormais séparé)
```
...
OPTION 2 — Mort RP :
  -> Le joueur en fait la demande VIA TICKET (confirmé, pas de contact direct GM)
...
- Le choix est **définitif** — sauf si le joueur paie **25 tokens** pour redistribuer un point
...
### Archétypes de Réincarnation — Idée en suspens (peut-être abandonnée)

> ⚠️ **Non décidé.** Cette idée n'est pas confirmée pour développement — elle pourrait ne jamais être implémentée. Documentée ici uniquement pour mémoire si le sujet est repris plus tard.

**Concept envisagé** : plutôt que de présenter le prestige comme une pure allocation de %, habiller chaque combinaison de branches en **archétype narratif** (ex: "Le Marchand", "Le Vétéran", "L'Érudit") — un titre affiché sur le profil du joueur, choisi à chaque wipe.

```
Principe envisagé :
  -> Le joueur choisit un NOUVEL archétype à chaque wipe
  -> L'archétype affiché (profil, RP) est REMPLACÉ à chaque nouvelle vie
  -> Les points de prestige (bonus chiffrés) restent CUMULATIFS,
     jamais perdus, peu importe l'archétype choisi
  -> Le titre raconte "qui il est maintenant", pas un contrat figé sur les stats
```

**Pourquoi c'est en suspens** : ajoute une couche de présentation RP intéressante mais non essentielle au fonctionnement du système — les 9 branches actuelles fonctionnent très bien sans cet habillage. À reprendre seulement si le temps/besoin le justifie.
...
### Archétypes de réincarnation — EN SUSPENS, pas confirmé

> ⚠️ **Idée non validée pour développement — sera peut-être abandonnée.** Documentée ici pour mémoire si le sujet est repris plus tard.

Concept envisagé : plutôt que d'afficher les branches/% bruts au joueur, habiller chaque choix de prestige d'un **titre narratif de réincarnation** (ex : "Le Marchand", "Le Vétéran", "L'Érudit"), visible sur le profil (`/profil`) pour renforcer l'immersion RP.

```
Fonctionnement envisagé :
  -> À chaque wipe, le joueur choisit un NOUVEL archétype/titre
  -> Le titre affiché REMPLACE l'ancien (représente l'identité de la vie actuelle)
  -> Les points de prestige investis restent CUMULATIFS et ne sont
     jamais perdus, peu importe l'archétype choisi à chaque vie
  -> Le titre est une étiquette narrative, pas un contrat figé sur les stats
```

> Objectif si implémenté : déplacer la décision du joueur de "j'optimise mon build" vers "je choisis qui je redeviens" — sans toucher au système de points déjà calibré.
...
Aucun point ouvert restant — mort RP demandée via ticket (confirmé), coût de redistribution fixé à 25 tokens (voir ci-dessus).

---

## Dépendances & Liaisons — Équilibrage

⚠️ **La branche Charisme (+5%/point sur les gains/pertes de réputation) dépend directement des valeurs de base de V1.CDC_ReputationModule.** Si les valeurs d'actions (vote +3, revive +4, etc.) changent, l'impact réel de Charisme change aussi — à revérifier ensemble.

⚠️ **La branche Prospérité (cashback au wipe) dépend indirectement de l'économie globale** (V1.CDC_EcoModule) — plus les joueurs accumulent d'argent, plus le cashback est significatif. Pas de calcul chiffré direct, mais à garder en tête si l'économie globale est rééquilibrée.

- **V1.CDC_ReputationModule** — Le prestige (Charisme) et les sanctions affectent/dépendent de la réputation
- **V1.CDC_PlayerModule** — Coffre Objets Perdus, mécaniques de wipe partagées
- **V1.CDC_Tebex** — Tokens utilisés pour le wipe prestige (250 tokens fixe)
- **V1.CDC_EcoModule** — Dépendance indirecte via la branche Prospérité
