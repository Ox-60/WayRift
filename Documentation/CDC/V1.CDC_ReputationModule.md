...
**Déclenchement : 7 jours sans connexion.** (Règle individuelle — distincte de l'inactivité de faction, voir V1.CDC_FactionModule pour le seuil à 40% de membres uniques connectés/semaine.)
...
- Accès au Bas-Fonds et marché noir (V3) — nécessite réputation individuelle Infâme ET appartenance à une faction d'alignement Infâme (voir V1.CDC_FactionModule)
...
|---|---|---|---|
| `/reputation info [joueur]` | Affiche réputation, label, historique récent | joueur | `wayrift.reputation.info` |
| `/reputation top` | Classement des joueurs | joueur | `wayrift.reputation.top` |
| `/reputation vote <joueur> up/down` | Utilise un vote disponible | joueur (si votes disponibles) | `wayrift.reputation.vote` |
| `/reputation set <uuid> <valeur>` | Définit la réputation directement | admin | `wayrift.reputation.admin.set` |
| `/reputation add <uuid> <valeur> <raison>` | Ajoute/retire des points avec log | admin/GM | `wayrift.reputation.admin.add` |
| `/reputation history <uuid>` | Historique complet des variations | admin/GM | `wayrift.reputation.admin.history` |
| `/reputation reload` | Recharge la configuration | admin | `wayrift.reputation.admin.reload` |
...
| # | Point | Priorité |
|---|---|---|
| 1 | ~~Avantages Infâmes détaillés~~ — **Voir V3.CDC_BasFonds.md (document dédié créé)** | ✅ |

---

## Dépendances & Liaisons

- **FactionModule** — Alignement de faction calculé sur la réputation moyenne des membres
- **PlayerModule** — vol/racket influencent la réputation selon des seuils précis
- **Permissions** — sanctions de modération (warn/mute/ban) affectent la réputation
- **Events_GM** — l'event Traque cible les joueurs selon leur réputation
