...
### Coût des recherches — Modèle évolutif par catégorie

**Confirmé : système de coût évolutif**, comme les jeux de stratégie/guildes mobiles (RoK, Lords Mobile) — chaque palier coûte plus cher que le précédent, et les catégories de recherche ont des coûts de base différents selon leur impact.

```
Coût palier N (catégorie X) = Coût de base de la catégorie × croissance^(N-1)

Catégories et coût de base relatif (exemple) :
  Économie (mineure)     : coût de base faible — bonus modérés (+5%)
  Identité (spécialisée) : coût de base moyen — bonus ciblés (réputation, combat, marché noir)
  Ultime (palier 5+)      : coût de base élevé — débloque Boutique avancée, teleport
```

> Valeurs exactes de coût de base et taux de croissance par catégorie — à chiffrer précisément une fois le reste de l'économie Nation calibré (point ouvert).
...
**Confirmé : une grande variété d'artefacts, avec noms uniques et forte valeur narrative** — pas juste des bonus stat génériques.

| Catégorie | Exemples de noms | Nature |
|---|---|---|
| Bijoux/Colliers | "Collier du Sage", "Pendentif des Abysses" | Bonus passif, très lié au lore |
| Armes décoratives | "Lame du Roi Déchu" | **Inutilisable en combat** — pièce d'exposition avec bonus passif |
| Armures décoratives | "Armure du Premier Gardien" | **Inutilisable en combat** — même principe, symbole plutôt qu'équipement |
| Gemmes | "Larme de Cristal", "Éclat Corrompu" | Petits objets, souvent en sets |
| Sets d'artefacts | "Regalia de l'Ancien Monde" (3 pièces) | Bonus supplémentaire si toutes les pièces de la famille sont réunies |

> Les armes/armures artefacts sont **volontairement inutilisables en combat** — ce sont des symboles de prestige et de lore, pas de l'équipement fonctionnel. Leur valeur est narrative et le bonus passif qu'ils accordent à la faction, pas leurs stats de combat.
...
- **NationModule** — système réservé exclusivement aux Nations
- **Gameplay Layers** — justification de la restriction aux Nations
