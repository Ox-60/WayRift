...
| Commande | Description | Nœud |
|---|---|---|
| `/gm traque start <durée>` | Lance l'event de traque avec durée en minutes | `wayrift.events.gm.traque.start` |
| `/gm traque setprime <montant>` | Définit la prime par tête Infâme | `wayrift.events.gm.traque.setprime` |
| `/gm traque setprime <uuid> <montant>` | Prime spéciale pour un joueur ciblé | `wayrift.events.gm.traque.setprime.target` |
| `/gm traque end` | Termine l'event manuellement | `wayrift.events.gm.traque.end` |
| `/gm traque status` | Affiche les stats en cours (kills, survivants) | `wayrift.events.gm.traque.status` |
| `/gm coma <uuid> <minutes>` | Place un joueur en coma prolongé | `wayrift.events.gm.coma` |
