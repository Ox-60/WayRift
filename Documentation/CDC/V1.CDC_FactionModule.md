...
- **Puissance** : combien la faction pèse militairement (via événements/guerres). ⚠️ **Ne détermine plus les claims** (remplacé par le système d'achat lié à la taille du groupe — voir section Claims ci-dessus). Son usage exact reste à définir : probablement un indicateur affiché (classement `/f top`), sans effet mécanique direct pour l'instant (point ouvert).
...
> ⚠️ **Point ouvert** : depuis que les claims sont achetés (liés à la taille du groupe, pas à la Puissance), la Puissance de Groupe/Organisation n'a plus de fonction mécanique directe. À définir : reste-t-elle un simple indicateur de classement/prestige (`/f top`), ou lui donne-t-on un nouvel usage (accès à du contenu, réductions, autre) ?
