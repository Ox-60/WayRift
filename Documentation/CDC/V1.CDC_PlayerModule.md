...
### Wipe — Ce que le joueur perd réellement

Le wipe (qu'il soit via coma, via 250 tokens + palier de niveau, ou via mort RP) fait perdre au joueur :
- **Son groupe/faction actuelle** — plus aucune appartenance
- **Son logement** (plot housing) — perdu, items transférés au coffre Objets Perdus du nouveau personnage
- **Toutes ses relations RP construites** — son ancienne identité "appartient" désormais à quelqu'un d'autre narrativement

```
Mécanique de reconnaissance partielle :
  Le joueur PEUT recroiser les mêmes joueurs qu'avant son wipe.
  Mais son ancienne vie ne lui appartient plus complètement :
  -> Un joueur peut dire "un de mes cousins m'a parlé de qui vous étiez"
  -> Mais ne peut PAS donner de détails précis sur certains sujets
  -> L'ancienne identité devient une rumeur floue, pas un accès direct
```

> **Intention de design** : le wipe est pensé pour pousser les joueurs à accepter la mort de leur personnage et à recommencer une nouvelle vie — plutôt que de s'accrocher indéfiniment au même personnage. Ça allonge la durée de vie du serveur en encourageant le renouvellement narratif plutôt que la stagnation.
...
|---|---|---|---|
| `/profil [joueur]` | Affiche le profil selon les droits | joueur | `wayrift.player.profil` |
| `/wipe me` | Déclenche le wipe (double confirmation) | joueur | `wayrift.player.wipe` |
| `/player revive <uuid>` | Revive un joueur | GM | `wayrift.player.admin.revive` |
| `/player gm-block <uuid>` | Bloque le choix hôpital/wipe | GM | `wayrift.player.admin.gmblock` |
| `/player wipe <uuid> <raison>` | Force un wipe | admin | `wayrift.player.admin.wipe` |
| `/player setreputation <uuid> <val>` | Définit la réputation directement | admin | `wayrift.player.admin.setreputation` |
...
### Vol de coffre — Mécanique item-based

Le vol de coffre fonctionne via un **item consommable dédié**, sur le même principe que l'inspection joueur (fouille) mais appliqué aux coffres dans les claims.

```
1. Le voleur utilise l'item consommable sur un coffre non autorisé
2. Le coffre révèle 75% de son contenu (aléatoire, non exhaustif)
3. Le voleur peut prendre jusqu'à 25% du contenu révélé
4. L'item est consommé après usage

Malus de réputation :
  -> -3 pts PAR PILE (stack) d'item prise (pas un malus fixe unique)
  -> Uniquement si le voleur a réputation > 300
  -> Aucun lien avec la réputation du propriétaire du coffre
```

> Voler plusieurs piles dans le même coffre coûte proportionnellement plus cher en réputation — un vol massif est plus risqué qu'un vol ciblé. Pourcentages (75%/25%) et malus par pile à valider/calibrer précisément (point ouvert).

- Détecté via CoreProtect → logs et malus automatiques
...
- **ReputationModule** — vol/racket influencent la réputation, seuils liés
- **Permissions** — coffre Objets Perdus alimenté par les pertes de logement/banque
- **EcoModule** — coûts de revive (hôpital, collègue) liés à l'économie globale
