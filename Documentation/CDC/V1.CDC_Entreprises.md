...
## Paramétrage — Prix géré manuellement par GM

> **Pas de fourchette fixe universelle.** Chaque item unique est différent en utilité — certains coûtent plus cher à fabriquer et se vendent plus cher, d'autres restent modestes. Le prix constructeur et la recette de craft sont **définis manuellement par un GM**, au cas par cas, lors de la création de l'item (via ticket staff).

| Paramètre | Gestion |
|---|---|
| Prix constructeur | Fixé manuellement par un GM à la création de l'item, adapté à son utilité et au coût réel de sa recette |
...
| Fourchette prix B2B | 125% à 150% du prix constructeur (fixé par le GM) |
...
|---|---|---|
| `/e admin create <organisation> <nom>` | Crée un nouvel item unique pour une organisation | GM |
| `/e admin setprice <item> <prix>` | Définit le prix constructeur de l'item | GM |
| `/e admin setrecipe <item>` | Ouvre l'éditeur de recette (ressources + quantités) | GM |
| `/e admin editprice <item> <prix>` | Modifie le prix constructeur d'un item existant (rééquilibrage) | GM |
